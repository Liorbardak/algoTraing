    ...
    for i, ticker in enumerate(target_tickers, 1):
        ...
        try:
            ...
            for call in valid_earnings:
                ...
                print(f"    Found {len([c for c in tentative_compliments if c.get('level', 0) > 0])} potential compliments")
            ...
            potential_compliments_count = sum(
                len([c for c in qd['compliments'] if c.get('level', 0) > 0]) 
            ...
            if potential_compliments_count > 0:
                ...
                if isinstance(batch_validated_response, list) and len(batch_validated_response) > 0:
                ...
                print(f"  Validation completed. Remaining compliments: {validation_applied_count}")
            else:
            ...
            if validation_successful:
                ...
                for quarter_key, quarter_data in validated_quarter_compliments.items():
                    ...
                    quarter_level_1 = sum(1 for c in compliments if c.get('level', 0) == 1)
                    quarter_level_2 = sum(1 for c in compliments if c.get('level', 0) == 2)
                    quarter_level_3 = sum(1 for c in compliments if c.get('level', 0) == 3)
...
def prepare_batch_validation_input(all_quarter_compliments):
    """Prepare batch validation input for all detected compliments across quarters."""
    batch_input = "BATCH VALIDATION - Please validate each compliment below:\n\n"
    batch_input += "For each compliment, return 1 if it contains a genuine analyst compliment or congratulation.\n"
    batch_input += "Return 0 if it contains company self-references (we are pleased, our performance) or is not a genuine compliment.\n\n"
    
    compliment_index = 1
    for quarter_key, quarter_data in all_quarter_compliments.items():
        for compliment in quarter_data['compliments']:
            if compliment.get('level', 0) > 0:  # Only include potential compliments
                batch_input += f"[{compliment_index}] Analyst: {compliment.get('analyst_name', 'Unknown')}\n"
                batch_input += f"Quarter: {quarter_key}\n"
                batch_input += f"Quote: \"{compliment.get('quoted_compliment', '')}\"\n"
                batch_input += f"Current Level: {compliment.get('level', 0)}\n\n"
                compliment_index += 1
    
    batch_input += "\nReturn a JSON array with validation results. For each compliment, return:\n"
    batch_input += "{'index': X, 'level': 0 or original_level, 'analyst_name': 'Name', 'quoted_compliment': 'quote or empty if invalid'}\n"
    
    return batch_input


def parse_batch_validation_response(validation_response, original_structure):
    """Parse the batch validation response and update the original structure."""
    # Start with the original structure to preserve ALL analysts
    updated_structure = {}
    for quarter_key, quarter_data in original_structure.items():
        updated_structure[quarter_key] = {
            'year': quarter_data['year'],
            'quarter': quarter_data['quarter'],
            'date': quarter_data['date'],
            'compliments': quarter_data['compliments'].copy()  # Copy all original analysts
        }
    
    # If validation response is available and is a list, apply validation results
    if isinstance(validation_response, list) and len(validation_response) > 0:
        print(f"    Applying {len(validation_response)} validation results...")
        
        # Create index mapping for validation results
        validation_by_index = {}
        for result in validation_response:
            if isinstance(result, dict) and 'index' in result:
                validation_by_index[result['index']] = result
        
        # Apply validation results using index mapping
        current_index = 1
        for quarter_key in sorted(original_structure.keys()):
            original_compliments = original_structure[quarter_key]['compliments']
            updated_compliments = updated_structure[quarter_key]['compliments']
            
            for i, original_compliment in enumerate(original_compliments):
                # If this analyst had a potential compliment
                if original_compliment.get('level', 0) > 0:
                    if current_index in validation_by_index:
                        validation_result = validation_by_index[current_index]
                        
                        # Apply validation result
                        validated_level = validation_result.get('level', 0)
                        updated_compliments[i]['level'] = validated_level
                        
                        # If validation failed, clear the quote
                        if validated_level == 0:
                            updated_compliments[i]['quoted_compliment'] = ''
                        else:
                            # Keep the validated quote if provided, otherwise keep original
                            validated_quote = validation_result.get('quoted_compliment', '')
                            if validated_quote:
                                updated_compliments[i]['quoted_compliment'] = validated_quote
