...
import json
import pandas as pd
from run_detection_only import run_detection_only
...
def run_iteration(run_name, prompt_path, target_tickers, prev_fp=None, prev_fn=None, iteration=1):
    results_path = os.path.join('detection_results', f"results_{run_name}")
    evaluation_path = os.path.join('detection_evaluation', f"evaluation_{run_name}")
...
def main():
    ...
    score, results_df, all_fp, all_fn = run_iteration('V1', improved_prompt_path_v1, target_tickers, all_fp, all_fn, iteration=1)
    scores.append(score)
    results_dfs.append(results_df)
    fp_list.append(all_fp)
    fn_list.append(all_fn)
    if False:
        ...
        score, results_df, all_fp, all_fn = run_iteration('V2', improved_prompt_path_v2, target_tickers, all_fp, all_fn, iteration=2)
        scores.append(score)
        results_dfs.append(results_df)
        fp_list.append(all_fp)
        fn_list.append(all_fn)
        ...
        with open(prompt_paths[-1], 'r', encoding='utf-8') as f:
            prev_prompt = f.read()
        improved_prompt_v3 = call_llm_to_improve_prompt(prev_prompt, all_fp, all_fn, iteration=3)
