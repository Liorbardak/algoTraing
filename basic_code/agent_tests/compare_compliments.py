...
def compare_analyst_compliments(actual, gt, before_validation):
        ...
        for actual_name, actual_c in actual_by_name.items():
                ...
                if actual_c['analyst_name'].strip().lower() == gt_c['analyst_name'].strip().lower():
                    ...
                    if actual_c['level'] == gt_c['level']:
                        ...
                        if actual_c['level'] == 0:
                            ...
                            gt_actual_quoted_compliment = gt_c['quoted_compliment'].split('____')[-1]
                            if len(gt_actual_quoted_compliment) == 0:
                                gt_actual_quoted_compliment= gt_c['quoted_compliment'].split('____')[0]
                            # Note - due to GT file structure - revisit
                            false_negative_phrases.append(gt_actual_quoted_compliment)
                        else:
                            stats['false_positive'] += 1
                            false_positive_phrases.append(actual_c['quoted_compliment'])
                        results.append({
                            'quarter': quarter,
            ...
            if best_gt_name:
                ...
                if actual_c['level'] == gt_c['level']:
                    ...
                    if actual_c['level'] == 0:
                        stats['false_negative'] += 1
                        gt_actual_quoted_compliment = gt_c['quoted_compliment'].split('____')[-1]  # Note - due to GT file structure - revisit
                        if len(gt_actual_quoted_compliment) == 0:
                            gt_actual_quoted_compliment = gt_c['quoted_compliment'].split('____')[0]
                        false_negative_phrases.append(gt_actual_quoted_compliment)
                    else:
    ...
    return results, results_incorrect_names , stats, false_negative_phrases , false_positive_phrases

def run_evaluation(tickers , resdir, outputdir):
    '''
    Run evaluation on detector results
    :param tickers: tickers to run on
    :param resdir: get results from this directory
    :param outputdir: export your results to this directory
    :return: score , outputfile path
    '''
    basepath = '../../../data/'
    ...
    res = []
    all_false_positive_phrases = []
    all_false_negative_phrases = []
    for ticker in tickers:
        # Set paths
        before_validation_file = os.path.join(resdir, f"{ticker}_validated_compliments.json")
        ...
        # Load json
        actual = load_json(before_validation_file)
        gt = load_json(gt_file)
        before_validation = load_json(before_validation_file)

        # Run compare
        results, results_incorrect_names, stats, false_negative_phrases , false_positive_phrases = compare_analyst_compliments(actual, gt, before_validation)
        all_false_positive_phrases += false_positive_phrases
        all_false_negative_phrases += false_negative_phrases
        ...
        print(
            f"Statistics for {ticker}: Correct: {stats['correct']}, Incorrect Level: {stats['incorrect_level']}, Incorrect Name: {stats['incorrect_name']}")
    ...
    score = df.loc['Total'].accuracy
    return score , df, all_false_positive_phrases , all_false_negative_phrases
...
def main():
    ...
    gtdir = os.path.join(basepath, "GT")
    outputdir = os.path.join(resdir,'compareToGT_temp')
    os.makedirs(outputdir, exist_ok=True)
    statistic_filename = os.path.join(outputdir, f"compareToGT.csv")
    tickers = ['ADMA', 'ADM', 'CLBT']
    tickers = ['CYBR']
    score , df, all_false_positive_phrases , all_false_negative_phrases = run_evaluation(tickers,resdir, outputdir)

    print(score)
    print('false positive phrases:')
    for phrase in all_false_positive_phrases:
        print('FP: ', phrase)
    print('false negative phrases:')
    for phrase in all_false_negative_phrases:
        print('FN: ', phrase)
